    ...
    <div className="fixed inset-0 z-50 bg-black/25">
      <div className="absolute right-0 top-0 h-full w-full max-w-[640px] overflow-y-auto rounded-l-2xl bg-[#f8f9fb] p-6 shadow-xl sm:p-8">
        <div className="mb-5 flex items-start justify-between">
          <h2 className="text-title text-[34px] font-semibold tracking-[-0.02em]">Task details</h2>
          <button
            type="button"
        ...
        <div className="space-y-6">
          <label className="block">
            <span className="mb-2 block text-[14px] font-medium text-[#90a0b8]">Task name</span>
            <input
              type="text"
              value={draft.title}
              onChange={(event) => onChange("title", event.target.value)}
              placeholder="Task name"
              className="w-full rounded-xl border border-[#3562df] bg-transparent px-4 py-3 text-[33px] font-medium tracking-[-0.02em] text-title outline-none focus:ring-2 focus:ring-[#3562df]/20"
            />
          </label>

          <label className="block">
            <span className="mb-2 block text-[14px] font-medium text-[#90a0b8]">Description</span>
            <textarea
              value={draft.description}
              onChange={(event) => onChange("description", event.target.value)}
              placeholder="Enter a short description"
              rows={4}
              className="w-full resize-none rounded-xl border border-[#cfd6e2] bg-transparent px-4 py-3 text-[30px] leading-[1.2] tracking-[-0.02em] text-title outline-none focus:border-[#3562df] focus:ring-2 focus:ring-[#3562df]/20"
            />
          </label>

          <section>
            <h3 className="mb-2 text-[14px] font-medium text-[#90a0b8]">Icon</h3>
          ...
          <section>
            <h3 className="mb-2 text-[14px] font-medium text-[#90a0b8]">Status</h3>
              ...
              {STATUS_OPTIONS.map((status) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-title text-[34px] font-medium tracking-[-0.02em]">
                      {status.label}
                    </span>
                    {active ? (
